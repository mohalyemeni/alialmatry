<?php $__env->startSection('content'); ?>
                        ...
                        <tr>
                            ...
                            <td>
                                <?php
                                    $thumbnailUrl = null;
                                    if ($video->thumbnail) {
                                        // First, check if it's an absolute URL (like from YouTube directly)
    if (
        Illuminate\Support\Str::startsWith($video->thumbnail, [
            'http://',
            'https://',
        ])
    ) {
    ...
    // Otherwise, assume it's a local path and build the full URL with asset()
                                        elseif (file_exists(public_path($video->thumbnail))) {
                                            $thumbnailUrl = asset($video->thumbnail);
                                        }
                                    }
                                ?>
                                <?php if($thumbnailUrl): ?>
                                    <img src="<?php echo e($thumbnailUrl); ?>" alt="<?php echo e($video->title); ?>" class="img-thumbnail"
                                        style="width: 81px; height: 70px; object-fit: cover; border-radius: 100%;">
                                <?php else: ?>
                                    <div class="d-flex align-items-center justify-content-center"
                                        style="width:120px; height:70px; background:#f5f5f5; border-radius: 5px;">
