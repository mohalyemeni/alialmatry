...
@section('content')
    <div class="card shadow mb-4">
        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                <h3 class="font-weight-bold text-primary">
                    ...
                    {{ __('panel.manage_blog_categories') }}
                </h3>
                <ul class="breadcrumb pt-3">
                    <li><a href="{{ route('admin.index') }}">{{ __('panel.main') }}</a> \</li>
                    <li class="ms-1">{{ __('panel.show_blog_categories') }}</li>
                </ul>
            </div>
        ...
        @include('backend.blog_categories.filter.filter')
                        ...
                        <tr>
                            ...
                            <td class="d-none d-sm-table-cell text-center">
                                <a href="javascript:void(0);" class="updateBlogCategoryStatus"
                                    id="blog-category-{{ $page_category->id }}"
                                    blog_category_id="{{ $page_category->id }}">
                            ...
                            <td class="d-none d-sm-table-cell text-center">
                                <a href="javascript:void(0);" class="toggleBlogCategoryFeatured"
                                    id="blog-category-featured-{{ $page_category->id }}"
                                    blog_category_id="{{ $page_category->id }}">
                                    ...
                                    <div class="dropdown mb-2">
                                        ...
                                        <div class="dropdown-menu"
                                            ...
                                            <a class="dropdown-item d-flex align-items-center"
                                                href="{{ route('admin.blog_categories.edit', $page_category->id) }}">
                                                <i data-feather="edit-2" class="icon-sm me-2"></i>
                                                <span>{{ __('panel.operation_edit') }}</span>
                                            </a>

                                            <a href="javascript:void(0);" class="dropdown-item d-flex align-items-center"
                                                onclick="confirmDelete('delete-blog-category-{{ $page_category->id }}', '{{ __('panel.confirm_delete_message') }}', '{{ __('panel.yes_delete') }}', '{{ __('panel.cancel') }}')">
                                                <i data-feather="trash" class="icon-sm me-2"></i>
                                                <span>{{ __('panel.operation_delete') }}</span>
                                            </a>

                                            <form id="delete-blog-category-{{ $page_category->id }}"
                                                action="{{ route('admin.blog_categories.destroy', $page_category->id) }}"
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.updateBlogCategoryStatus', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('admin.blog_categories.toggleStatus') }}',
                    data: {
                        _token: '{{ csrf_token() }}',
            ...
            $(document).on('click', '.toggleBlogCategoryFeatured', function() {
                ...
                $.ajax({
                    type: 'POST',
                    url: '{{ route('admin.blog_categories.toggleFeatured') }}',
                    data: {
                        _token: '{{ csrf_token() }}',
                        category_id: category_id
                    },
