        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                ...
                <ul class="breadcrumb pt-3">
                    ...
                    <li class="ms-1">
                        <a href="{{ route('admin.blog_categories.index') }}">{{ __('panel.manage_blog_categories') }}</a> /
                    </li>
            ...
            <form action="{{ route('admin.blog_categories.update', $category->id) }}" method="post"
                ...
                <div class="tab-content" id="myTabContent">
                    <div class="tab-pane fade show active" id="content" role="tabpanel" aria-labelledby="content-tab">
                            ...
                            <div class="col-sm-12 col-md-10 pt-3 d-flex align-items-center gap-3">
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_active"
                                        value="1"
                                        {{ (string) old('status', $category->status) === '1' ? 'checked' : '' }}>
                                ...
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_inactive"
                                        value="0"
                                        {{ (string) old('status', $category->status) === '0' ? 'checked' : '' }}>
                        ...
                        <div class="row mb-3">
                            <div class="col-sm-12 col-md-2 pt-3">
                                <label for="featured" class="control-label">{{ __('panel.featured') ?? 'مميز' }}</label>
                            </div>
                            <div class="col-sm-12 col-md-10 pt-3">
                                {{-- hidden input + star button --}}
                                <input type="hidden" name="featured" id="featured_input"
                                    value="{{ (int) old('featured', $category->featured) ? 1 : 0 }}">
                                <button type="button" id="featured_btn" class="btn btn-light p-2"
                                    aria-pressed="{{ (int) old('featured', $category->featured) ? 'true' : 'false' }}">
                                    <i id="featured_icon"
                                        class="{{ (int) old('featured', $category->featured) ? 'fas fa-star text-warning' : 'far fa-star text-muted' }}"
                        ...
                        <div class="row">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <input type="text" name="meta_slug" id="meta_slug"
                                    value="{{ old('meta_slug', $category->meta_slug) }}" class="form-control">
                                @error('meta_slug')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-sm-12 col-md-10 pt-3">
                            ...
                            <a href="{{ route('admin.blog_categories.index') }}" class="btn btn-outline-danger">
    ...
    <script>
        $(function() {
            $("#img").fileinput({
                ...
                initialPreview: [
                    @if ($category->img)
                        "{{ asset('assets/blog_categories/' . $category->img) }}"
                    @endif
                ],
                    ...
                    @if ($category->img)
                        {
                            ...
                            url: "{{ route('admin.blog_categories.remove_image') }}",
                            key: "{{ $category->id }}",
                            extra: {
                                _token: "{{ csrf_token() }}",
                                key: "{{ $category->id }}"
