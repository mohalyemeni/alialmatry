        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                <h3 class="font-weight-bold text-primary">
                    <i class="fa fa-edit"></i> {{ __('panel.edit_audio') }}
                </h3>
                ...
                <div class="tab-content" id="myTabContent">
                                ...
                                <select name="category_id" id="category_id"
                                    ...
                                    @foreach ($categories as $category)
                                        <option value="{{ $category->id }}"
                                            {{ old('category_id', $audio->category_id) == $category->id ? 'selected' : '' }}>
                                            {{ $category->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                <textarea name="description" id="description" rows="10"
                                    class="form-control summernote @error('description') is-invalid @enderror">{!! old('description', $audio->description) !!}</textarea>
                                @error('description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                @if ($audio->audio_file)
                                    <audio controls style="width: 100%; max-width: 400px;">
                                        <source src="{{ asset('assets/audios/files/' . $audio->audio_file) }}"
                                            type="audio/mpeg">
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                <div class="input-group flatpickr" id="flatpickr-datetime">
                                    <input type="text" name="published_on"
                                        value="{{ old('published_on', $audio->published_on?->format('Y-m-d H:i')) }}"
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_active"
                                        value="1" {{ old('status', $audio->status) == '1' ? 'checked' : '' }}
                                        required>
                                ...
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_inactive"
                                        value="0" {{ old('status', $audio->status) == '0' ? 'checked' : '' }}
                                        required>
                    ...
                    <div class="tab-pane fade" id="SEO" role="tabpanel" aria-labelledby="SEO-tab">
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <input type="text" name="meta_slug" id="meta_slug"
                                    value="{{ old('meta_slug', $audio->meta_slug) }}" class="form-control">
                                @error('meta_slug')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <input name="meta_keywords" id="tags1"
                                    value="{{ old('meta_keywords', $audio->meta_keywords) }}" class="form-control" />
                                @error('meta_keywords')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <textarea name="meta_description" id="meta_description" rows="3" class="form-control">{{ old('meta_description', $audio->meta_description) }}</textarea>
    ...
    <script>
        $(function() {
            $("#img").fileinput({
                ...
                initialPreview: [
                    @if ($audio->img)
                        "{{ asset('assets/audios/images/' . $audio->img) }}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    @if ($audio->img)
                        {
                            caption: "{{ basename($audio->img) }}",
                            url: "{{ route('admin.audios.remove_image') }}",
                            key: "{{ $audio->id }}",
                            extra: {
                                _token: "{{ csrf_token() }}",
                                id: "{{ $audio->id }}"
            ...
            $("#audio_file").fileinput({
                ...
                initialPreviewConfig: [
                    @if ($audio->audio_file)
                        {
                            caption: "{{ basename($audio->audio_file) }}",
                            url: "{{ route('admin.audios.remove_audio') }}",
                            key: "{{ $audio->id }}",
                            extra: {
                                _token: "{{ csrf_token() }}",
                                id: "{{ $audio->id }}"
