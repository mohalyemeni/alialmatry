    ...
    <script>
        $(document).ready(function() {
            ...
            $form.on('submit', function(e) {
                ...
                $btnText.html(
                    '<span class="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span> جاري الرفع...'
                );
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        $progress.removeClass('progress-bar-animated bg-primary').addClass(
                            'bg-success');
                        $status.html(
                                '<i class="fa fa-check-circle me-1"></i> تم رفع الملفات بنجاح')
                    ...
                    error: function(err) {
                        $progress.removeClass('progress-bar-animated bg-primary').addClass(
                            'bg-danger');
                        $status.html(
                            '<i class="fa fa-times-circle me-1"></i> حدث خطأ أثناء الرفع. يرجى المحاولة مجددًا'
