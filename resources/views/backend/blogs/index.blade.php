        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                ...
                <ul class="breadcrumb pt-3">
                    ...
                    <li class="ms-1">{{ __('panel.show_blogs') }}</li>
                </ul>
            </div>
            <div class="ml-auto">
                @ability('admin', 'create_blog')
                    ...
                    @forelse ($blogs as $blog)
                        <tr>
                            <td class="text-center">
                                <input type="checkbox" name="checkfilter" value="{{ $blog->id }}">
                            </td>
                            <td>{{ $blog->title }}</td>
                            <td class="d-none d-sm-table-cell">
                                {{ $blog->creator?->first_name ?? __('panel.unknown') }}
                            </td>
                            <td class="d-none d-sm-table-cell">
                                <a href="javascript:void(0);" class="updateBlogStatus" id="blog-{{ $blog->id }}"
                                    blog_id="{{ $blog->id }}">
                                    @if ($blog->status)
                            ...
                            <td class="d-none d-sm-table-cell">
                                {{ $blog->published_on?->diffForHumans() ?? '-' }}
                            </td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    <div class="dropdown mb-2">
                                        <a class="d-flex align-items-center" href="#"
                                            id="dropdownMenuButton{{ $blog->id }}" data-bs-toggle="dropdown"
                                        ...
                                        <div class="dropdown-menu" aria-labelledby="dropdownMenuButton{{ $blog->id }}">
                                            ...
                                            <a href="javascript:void(0);" class="dropdown-item d-flex align-items-center"
                                                onclick="confirmDelete('delete-blog-{{ $blog->id }}', '{{ __('panel.confirm_delete_message') }}')">
