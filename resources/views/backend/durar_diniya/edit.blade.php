        ...
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                <h3 class="font-weight-bold text-primary">
                    <i class="fa fa-edit"></i> {{ __('panel.edit_durar_diniya') }}
                </h3>
                <ul class="breadcrumb pt-3">
                    <li><a href="{{ route('admin.index') }}">{{ __('panel.home') }}</a> /</li>
                    <li class="ms-1"><a
                            href="{{ route('admin.durar_diniya.index') }}">{{ __('panel.manage_durar_diniya') }}</a> /</li>
                    <li class="ms-1">{{ $item->title }}</li>
                </ul>
            </div>
        </div>
        <div class="card-body">
            ...
            <form action="{{ route('admin.durar_diniya.update', $item->id) }}" method="POST" enctype="multipart/form-data"
                novalidate>
                @csrf
                @method('PUT')
                ...
                <div class="tab-content" id="myTabContent">
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                <div class="input-group flatpickr" id="flatpickr-datetime">
                                    <input type="text" name="published_on"
                                        value="{{ old('published_on', $item->published_on?->format('Y-m-d H:i')) }}"
                            ...
                            <div class="col-sm-12 col-md-10 pt-3">
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_active"
                                        value="1" {{ old('status', $item->status) == '1' ? 'checked' : '' }}>
                                ...
                                <div class="form-check form-check-inline">
                                    <input type="radio" class="form-check-input" name="status" id="status_inactive"
                                        value="0" {{ old('status', $item->status) == '0' ? 'checked' : '' }}>
                    ...
                    <div class="tab-pane fade" id="SEO" role="tabpanel" aria-labelledby="SEO-tab">
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <input type="text" name="meta_slug" id="meta_slug"
                                    value="{{ old('meta_slug', $item->meta_slug) }}" class="form-control">
                                @error('meta_slug')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <input name="meta_keywords" id="tags1"
                                    value="{{ old('meta_keywords', $item->meta_keywords) }}" class="form-control" />
                                @error('meta_keywords')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="row mt-3">
                            ...
                            <div class="col-sm-12 col-md-9 pt-3">
                                <textarea name="meta_description" id="meta_description" rows="3" class="form-control">{{ old('meta_description', $item->meta_description) }}</textarea>
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-sm-12 col-md-10 pt-3">
                        ...
                        <a href="{{ route('admin.durar_diniya.index') }}" class="btn btn-outline-danger">
    ...
    <script>
        $(function() {
            $("#img").fileinput({
                ...
                initialPreview: [
                    @if ($item->img)
                        "{{ asset('assets/durar_diniya/images/' . $item->img) }}"
                    @endif
                ],
                ...
                initialPreviewConfig: [
                    @if ($item->img)
                        {
                            caption: "{{ basename($item->img) }}",
                            url: "{{ route('admin.durar_diniya.remove_image') }}",
                            key: "{{ $item->id }}",
                            extra: {
                                _token: "{{ csrf_token() }}",
                                id: "{{ $item->id }}"
            ...
            if ($('#flatpickr-datetime').length) {
                const defaultDate = "{{ old('published_on', $item->published_on?->format('Y-m-d H:i')) }}" ||
                    new Date();
