...
@section('content')
    <div class="card shadow mb-4">
        <div class="card-header py-3 d-flex justify-content-between">
            <div class="card-naving">
                <h3 class="font-weight-bold text-primary">
                    <i class="fa fa-book"></i> {{ __('panel.manage_durar_diniya') }}
                </h3>
                <ul class="breadcrumb pt-3">
                    <li><a href="{{ route('admin.index') }}">{{ __('panel.main') }}</a> \</li>
                    <li class="ms-1">{{ __('panel.show_durar_diniya') }}</li>
                </ul>
            </div>
            <div class="ml-auto">
        ...
        @includeIf('backend.durar_diniya.filter.filter')
        ...
        <div class="card-body">
            <table id="datatable-buttons" class="table table-striped table-bordered dt-responsive nowrap"
                ...
                <tbody>
                    @forelse ($durars as $durar)
                        <tr>
                            <td class="text-center"><input type="checkbox" name="checkfilter" value="{{ $durar->id }}">
                            </td>
                            <td>{{ $durar->title }}</td>
                            ...
                            <td class="d-none d-sm-table-cell">{{ $durar->published_on?->diffForHumans() ?? '-' }}</td>
                            <td class="text-center">
                                <div class="btn-group btn-group-sm">
                                    <div class="dropdown mb-2">
                                        <a class="d-flex align-items-center" href="#"
                                            id="dropdownMenuButton{{ $durar->id }}" data-bs-toggle="dropdown"
            ...
            <div class="mt-3">{{ $durars->links() }}</div>
        </div>
    </div>
@endsection
